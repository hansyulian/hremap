...
pub enum RawAction {
    Key { keys: Vec<String> },
    Macro { mode: MacroMode, steps: Vec<RawMacroStep> },
    Layer { layer: String, mode: LayerMode },
    Volume {
...
pub enum Action {
    Key(KeyCombo),
    Macro { mode: MacroMode, steps: Vec<MacroStep> },
    Layer { layer: String, mode: LayerMode },
    Volume { direction: VolumeDirection, amount: f32 },
    AppVolume { direction: VolumeDirection, amount: f32 },
...
fn resolve_action(raw: &RawAction) -> Result<Action> {
    match raw {
        RawAction::Key { keys } => {
            Ok(Action::Key(parse_combo(keys)?))
        }
        RawAction::Macro { mode, steps } => {
            let resolved_steps = steps.iter().map(|s| {
                Ok(MacroStep {
            ...
            Ok(Action::Macro { mode: mode.clone(), steps: resolved_steps })
        }
        RawAction::Layer { layer, mode } => {
            Ok(Action::Layer { layer: layer.clone(), mode: mode.clone() })
        }
        RawAction::Volume { direction, amount } => {
            Ok(Action::Volume { direction: direction.clone(), amount: *amount })
        }
        RawAction::AppVolume { direction, amount } => {
            Ok(Action::AppVolume { direction: direction.clone(), amount: *amount })
        }
        RawAction::Launch { command } => {
            Ok(Action::Launch { command: command.clone() })
        }
    }
}
...
pub fn load(path: &str) -> Result<Config> {
    ...
    for name in raw.layers.keys() {
        ...
        layers.insert(name.clone(), ResolvedLayer {
            name: name.clone(),
            mappings,
        });
    }
